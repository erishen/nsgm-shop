...
/**
 * Category DataLoader
 * 针对 category 表的批量数据加载器，解决 N+1 查询问题
 */
export class CategoryDataLoader {
  ...
  constructor() {
    ...
    this.byId = new DataLoader(
      async (ids: readonly number[]) => {
        try {
          console.log(`🔍 DataLoader: 批量加载 ${ids.length} 个 category by ID`);
          
          const placeholders = ids.map(() => '?').join(',');
          const sql = `SELECT id, name, icon, parent_id, sort_order, status, create_date, update_date FROM category WHERE id IN (${placeholders})`;
    ...
    this.byName = new DataLoader(
      async (names: readonly string[]) => {
        try {
          console.log(`🔍 DataLoader: 批量加载 ${names.length} 个 category by name`);
          
          const placeholders = names.map(() => '?').join(',');
          const sql = `SELECT id, name, icon, parent_id, sort_order, status, create_date, update_date FROM category WHERE name IN (${placeholders})`;
        ...
        try {
          ...
          const results = await Promise.all(
            searchTerms.map(async (term) => {
              const sql = 'SELECT id, name, icon, parent_id, sort_order, status, create_date, update_date FROM category WHERE name LIKE ?';
    ...
    // 按 parent_id 批量加载相关的 category
    this.byParentId = new DataLoader(
      async (parent_ids: readonly number[]) => {
        try {
          console.log(`🔍 DataLoader: 批量加载 ${parent_ids.length} 个 category by parent_id`);
          
          const placeholders = parent_ids.map(() => '?').join(',');
          const sql = `SELECT id, name, icon, parent_id, sort_order, status, create_date, update_date FROM category WHERE parent_id IN (${placeholders})`;
          
          const results = await executeQuery(sql, [...parent_ids]);
          
          // 按外键分组
          return parent_ids.map(parent_id => 
            results.filter((row: any) => row.parent_id === parent_id)
          );
        } catch (error) {
          console.error('DataLoader byParentId 批量加载失败:', error);
          throw error;
        }
      },
      {
  ...
  clearAll(): void {
    ...
    console.log('🧹 Category DataLoader 缓存已清空');
  }
...
/**
 * 创建 Category DataLoader 实例
 */
export function createCategoryDataLoader(): CategoryDataLoader {
  return new CategoryDataLoader();
}